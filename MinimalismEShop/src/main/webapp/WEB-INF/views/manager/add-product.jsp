			...
			<div class="jumbotron">
				<form:form id="frmThemSP" autocomplete="off" method="POST"
					modelAttribute="product" class="form-horizontal" role="form"
					  action="/example/import/new" enctype="multipart/form-data">
							...
							<form:select path="category.idCategory" class="combobox form-control" name="idCategory">
								...
								<c:forEach var="categ" items="${category }">
									<option value="${categ.idCategory }"
										<c:if test="${categ eq product.category}">						
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#frmThemSP").on('submit', function(e) {
			...
			if (namePrd.length == 0) {
				$("#nameProduct").parent().addClass('has-error');
				submitted = false;
			}
			var size = $("#size").val();
			if (size.length == 0) {
			...
			if (imgPath.length == 0) {
				$("#imgPath").parent().addClass('has-error');
				submitted = false;
			}
			var age = $("#age").val();
			if (age.length == 0) {
		...
		$(".combobox-selected").click(function() {
			$(this).parent().removeClass('has-error');
		});
	});
</script> -->
